...
oc login https://openshift.adorsys.de:443 --username=${env.OPENSHIFT_USER} --password=${env.OPENSHIFT_PASSWORD}
docker login -u $(oc whoami) -p $(oc whoami -t) https://openshift-registry.adorsys.de
docker build -t test:${env.TRAVIS_COMMIT} --build-arg JAR_FILE=test-$PROJECT_VERSION.jar .
